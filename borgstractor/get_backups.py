"""Classes and functions for retrieving backup information and files."""
import os
import subprocess
import datetime
import re
...
class DatedInfo:
    ...
    def pretty_date(self):
        ...
        elif self.date_time.date() == (datetime.date.today()
...
class Backup(DatedInfo):
    ...
    def mount(self):
        ...
        # mount the backup
        borg_command.create('run', 'mount', self.name)
    ...
    def extract_file(
        ...
        backup_list = (make_list.communicate()[0].decode(sys.stdout.encoding),
                       make_list.returncode)
        ...
        # catch errors
        if int(backup_list[1]) != 0:
            catch_borg_errors(backup_list)

        # searches for initial list of files, creates objects, prints, returns
        # array
        ...
        all_files = parse_file_info(found_files)
        print(all_files)
    ...
    def extract(self, num, file_list):
        ...
        extracted_file_name = (
            ext_dir + '/' +
            to_extract.date_time.strftime("%Y-%m-%d_%H:%M:%S-") +
            os.path.basename(full_extract))
        shutil.move(full_extract, extracted_file_name)

        # wait for user confirmation to open restored file
        input("Press [enter] to see your extracted file.")
...
def parse_file_info(file_array):
    ...
    for f in file_array:
        ...
        # find date
        raw_date = re.search(
            r'\s\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}', f).group()
...
def parse_backup_info(backup_array):
    ...
    for backup in backup_array:
        ...
        raw_date = re.search(
            r'\s\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}', backup).group()
        date_time = datetime.datetime.strptime(raw_date, ' %Y-%m-%d %H:%M:%S')
...
def done_mounting(mountpoint, opencommand):
    """Check if user found file to determine next action."""
    print("Your backup is available at {}. It will now open for you to find"
          " the files you need and copy them out of the backup. When you are"
          " done please return to this window to close the backup."
          ).format(mountpoint)
    # wait for user
...
def print_found_files(file_list):
    ...
    while i < len(file_list):
        ...
        output = ['(' + str(i + 1) + ')',
                  f.name, 'LAST MODIFIED ' + f.pretty_date()]
...
def catch_borg_errors(ret):
    ...
    messages = {
        'LockTimeout': ('Cannot unlock repository. Backup may be in progress.'
                        ' Try again in a few minutes.'),
        'passphrase': ('Passphrase was rejected. Update config file '
                       ' and try again.'),
        'valid': ('The repository in your config file does not appear '
                  ' to be valid. Please correct it.'),
        'exist': ('The repository in your config file does not exist.'
                  ' Please correct.'),
        'remote': ('Connection to backup server failed. '
                   ' Check network connection.'),
        'other': ('An unknown error has occurred. '
                  ' Printing borg traceback:\n' + ret[0]),
    }
